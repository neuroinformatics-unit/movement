...
def test_from_mmpose_file(tmp_path):
    ...
    assert ds.position.shape == (2, 2, 2, 2)  # (time, space, keypoints, individuals)
    assert ds.confidence.shape == (2, 2, 2)   # (time, keypoints, individuals)
    assert np.allclose(ds.position.sel(time=0, space="x", individuals="id_0"), [10, 30])
    assert np.allclose(ds.confidence.sel(time=1, individuals="id_1"), [0.75, 0.65])
...
def test_from_freemocap_dir(tmp_path):
    ...
    assert isinstance(ds, xr.Dataset)
    assert ds.position.shape == (2, 3, 2, 1) # (time, space, keypoints, individuals)
    assert "kp1" in ds.keypoints
    assert np.allclose(ds.position.sel(time=1, space="z", keypoints="kp2"), 6.1)
