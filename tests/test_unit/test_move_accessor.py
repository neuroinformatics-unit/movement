...
def test_compute_kinematics_with_invalid_dataset(
    invalid_poses_dataset, kinematic_property
):
    ...
    with pytest.raises(expected_exception):
        getattr(invalid_poses_dataset.move, f"compute_{kinematic_property}")()
...
def test_invalid_method_call(valid_dataset, method, request):
