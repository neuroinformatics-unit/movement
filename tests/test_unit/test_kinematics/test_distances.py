...
@pytest.mark.parametrize(
    "selection_fn",
    [
        # individual dim is scalar,
        # poses: multiple keypoint
        # bboxes: missing keypoint dim
        # e.g. comparing 2 individual from the same data array
        lambda position: (
            position.isel(individual=0),
            position.isel(individual=1),
        ),
        # individual dim is 1D
        # poses: multiple keypoint
        # bboxes: missing keypoint dim
        ...
        # both individual and keypoint dims are scalar (poses only)
        # e.g. comparing 2 individual from the same data array,
