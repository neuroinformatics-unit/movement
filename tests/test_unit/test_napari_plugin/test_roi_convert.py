...
def test_ellipse_returns_polygon_and_warns(ellipse_yx):
    with pytest.warns(
        UserWarning, match="approximated as a PolygonOfInterest"
    ):
...
def test_ellipse_approximate_bounds(
    ...
    with pytest.warns(UserWarning):
        roi = napari_shape_to_roi(
            ellipse_yx, "ellipse", ellipse_n_vertices=256
        )
    minx, miny, maxx, maxy = roi.region.bounds
