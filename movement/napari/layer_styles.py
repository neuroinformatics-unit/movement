...
class PointsStyle(LayerStyle):
    ...
    face_color: str | None = None  # Color of the face
    face_color_cycle: list[tuple] | None = None  # Cycle of colors
    face_colormap: str = DEFAULT_COLORMAP  # Default colormap
    text: dict = field(
        default_factory=lambda: {"visible": False}
    )  # Text settings
    ...
    def set_color_by(self, prop: str, cmap: str | None = None) -> None:
        ...
        n_colors = len(
            self.properties[prop].unique()
        )  # Get unique values count
        self.face_color_cycle = _sample_colormap(
            n_colors, cmap
        )  # Generate colors
