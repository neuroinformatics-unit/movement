...
napari's ``EventEmitter`` holds bound methods weakly, so callbacks owned
by a widget go silently dead once that widget is destroyed (e.g. when
the movement panel is closed), leaving the layers unsynced. A plain
function (or a ``partial`` of one) is held strongly, so the connection
lives as long as the layer or viewer it acts on.
"""
...
# Viewers already subscribed to the viewer-level callbacks below.
# Only a "have we wired this one?" guard; it keeps no viewer alive.
_WIRED_VIEWERS: WeakSet = WeakSet()


def ensure_layer_wiring(viewer) -> None:
    """Idempotently subscribe a viewer to the movement layer callbacks.

    ``partial`` objects are not bound methods, so napari holds them
    strongly and these subscriptions last as long as the viewer, no
    matter which widget requested them.
    """
    # Unwrap napari's public-only proxy, so that two proxies around the
    # same viewer are not wired twice.
