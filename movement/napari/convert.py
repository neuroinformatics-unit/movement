...
def napari_layers_to_ds(
    ...
    ----------
    napari_layers: np.ndarray
        Napari layer data generated by ``movement.napari.convert.ds_to_napari_layers``.
        This can be either a Tracks array representing pose data,
        with columns ``track_id, time, y, x``, or a Shapes array
        representing bounding box data (rectangle vertices).
    properties: pd.DataFrame
        DataFrame containing metadata associated with the napari layer.
        It contains ``individual``, ``time``, ``confidence`` and
        optionally ``keypoint``.
    fps: float | None
        frames per second. Default to None, so ``time`` coordinates will be 
        in frame numbers. 
