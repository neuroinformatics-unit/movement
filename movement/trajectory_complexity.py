...
def compute_sinuosity(
    ...
    for i in range(0, n_time - window_size + 1, stride):
        ...
        displacements = compute_displacement(window_data).isel(time=slice(1, None))
...
def compute_angular_velocity(
    ...
    dot_products = (unit_displacement.isel(time=slice(1, None)) *
                   unit_displacement.isel(time=slice(0, -1))).sum(dim="space")
...
def compute_tortuosity(
    ...
    if method == "angular_variance":
        ...
        if len(scales) < 2:
            log_warning("Not enough data points to compute fractal dimension. Returning NaN.")
            result = xr.full_like(
        ...
        dims = [dim for dim in data_filtered.dims if dim not in ["time", "space"]]
