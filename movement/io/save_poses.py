...
def _auto_split_individual(ds: xr.Dataset) -> bool:
    """Return True if there is only one individual in the dataset."""
    n_individual = ds.sizes["individual"]
    return n_individual == 1
...
def to_dlc_style_df(
    ds: xr.Dataset, split_individual: bool = False
    ...
    "scorer", "bodyparts", "coords" (if split_individual is True),
    ...
    individual = ds.coords["individual"].data.tolist()
    if split_individual:
        df_dict = {}
        for indiv in individual:
            individual_data = ds.sel(individual=indiv)
        ...
        index_levels = ["scorer", "individual", "bodyparts", "coords"]
        columns = pd.MultiIndex.from_product(
            [scorer, individual, bodyparts, coords], names=index_levels
        )
...
def to_dlc_file(
    ds: xr.Dataset,
    file_path: str | Path,
    split_individual: bool | Literal["auto"] = "auto",
) -> None:
    ...
    file_path : pathlib.Path or str
        ...
        must be either .h5 (recommended) or .csv.
    split_individual : bool or "auto", optional
        Whether to save individual to separate files or to the same file
        (see Notes). Defaults to "auto".

    Notes
    -----
    If ``split_individual`` is True, each individual will be saved to a
    ...
    "/path/to/filename_individual1.h5". If False, all individual will be
    ...
    individual in the dataset: True if there is only one, False otherwise.
    ...
    if split_individual == "auto":
        split_individual = _auto_split_individual(ds)

    elif not isinstance(split_individual, bool):
        raise logger.error(
            ValueError(
                "Expected 'split_individual' to be a boolean or 'auto', "
                f"but got {type(split_individual)}."
            )
        )

    if split_individual:
        # split the dataset into a dictionary of dataframes per individual
        df_dict = to_dlc_style_df(ds, split_individual=True)
        ...
        # convert the dataset to a single dataframe for all individual
        df_all = to_dlc_style_df(ds, split_individual=False)
...
def to_lp_file(
    ...
    with ``split_individual=True``. This setting means that each individual
    ...
    ValidPosesInputs.validate(ds)
    to_dlc_file(ds, valid_path, split_individual=True)
...
def to_sleap_analysis_file(ds: xr.Dataset, file_path: str | Path) -> None:
    ...
    # Target shapes:
    # "track_occupancy"     n_frames * n_individual
    # "tracks"              n_individual * n_space * n_keypoint * n_frames
    # "track_names"         n_individual
    # "point_scores"        n_individual * n_keypoint * n_frames
    # "instance_scores"     n_individual * n_frames
    # "tracking_scores"     n_individual * n_frames
    individual_names = ds.individual.values.tolist()
    n_individual = len(individual_names)
    ...
    # Mask denoting which individual are present in each frame
    ...
    instance_scores = np.full((n_individual, n_frames), np.nan, dtype=float)
    tracking_scores = np.full((n_individual, n_frames), np.nan, dtype=float)
...
def to_nwb_file(
    ...
    pynwb.file.NWBFile or list[pynwb.file.NWBFile]
        ...
        will be returned. If the dataset contains multiple individual,
    ...
    Create NWBFiles with custom metadata shared across individual.
    ...
    metadata for different keypoint (e.g. ``leftear``, ``rightear``):
    ...
    individual = ds.individual.values.tolist()
    if isinstance(individual, str):
        individual = [individual]
    is_multi_ind = len(individual) > 1
    subjects = {
    ...
    for nwb_file, id in zip(nwb_files, individual, strict=True):
