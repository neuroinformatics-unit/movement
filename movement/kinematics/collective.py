...
def _compute_mean_angle(
    ...
    mean_angle = xr.where(
        angle_defined,
        np.arctan2(vector_sum.sel(space="y"), vector_sum.sel(space="x")),
        np.nan,
    )
    if in_degrees:
...
def _compute_heading_from_keypoints(
    ...
    When called via ``body_axis_keypoints=(origin, target)``, origin is
    the posterior keypoint and target is the anterior keypoint, so the
    resulting vector points posterior → anterior.
    """
