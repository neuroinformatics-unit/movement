...
- `keypoint`, with size equal to the number of tracked keypoints per individual.
...
- `keypoint` is likewise labelled with a list of unique body part names, e.g. `snout`, `right_ear`, etc. Note that this dimension only exists in the poses dataset.
- `individual` is labelled with a list of unique names (e.g. `mouse1`, `mouse2`, etc. or `id_0`, `id_1`, etc.).
...
- `position`: the 2D or 3D locations of the keypoints over time, with shape (`time`, `space`, `keypoint`, `individual`).
