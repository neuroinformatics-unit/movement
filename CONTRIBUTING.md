...
The API reference is auto-generated by the `docs/make_api_index.py` script, and the [sphinx-autodoc](sphinx-doc:extensions/autodoc.html) and [sphinx-autosummary](sphinx-doc:extensions/autosummary.html) extensions.
...
For referencing movement objects in the docstrings, use the
`` :role:`<target>` ``
syntax with the appropriate [Python object role](sphinx-doc:domains/python.html#cross-referencing-python-objects).
For example, to link to <project:#movement.io.load_poses>, you can use:
```
:py:func:`movement.io.load_poses`
```
For referencing external Python modules using [intersphinx](sphinx-doc:extensions/intersphinx.html), you will need to define the mapping between module names and their documentation URLs in [`intersphinx_mapping`](sphinx-doc:extensions/intersphinx.html#confval-intersphinx_mapping) in `docs/source/conf.py`, if not already defined.
Once the module is included in the mapping, you can use the same `` :role:`<target>` `` syntax as above.
For example, to link to <inv:#xarray.Dataset.update>, you can use:
```
...
If you want to reference movement or other external Python objects in your examples, you can use the `` :role:`<target>` ``
syntax with the appropriate [Python object role](sphinx-doc:domains/python.html#python-xref-roles) to automatically link them to the relevant documentation. For more details, see the [Cross-referencing Python objects](#cross-referencing-python-objects) section.
