...
* Use {meth}`logger.info()<loguru._logger.Logger.info>` for informational messages where default values are assigned to optional parameters, e.g. `individual_names`, `keypoint_names` in {class}`ValidPosesInputs<movement.validators.datasets.ValidPosesInputs>`. These are expected behaviors, not indications of problems.
